...
async def startup_event():
    ...
    if database_url and asyncpg:
        # Create a small asyncpg pool for connection reuse in serverless/fast startup environments
    ...
    for key, name in (("page", "page"), ("per_page", "per_page")):
        if has_value(key):
            ...
            if key == "page":
                ...
                _ = _validate_positive_int(name, v, min_value=1, max_value=200)
            params[key] = v
...
async def api_meta():
    ...
    if pool:
        try:
            ...
            if not row:
                return JSONResponse({"status": "unknown", "last_successful_scrape_at": None})
            # asyncpg maps jsonb -> Python types
            return JSONResponse(content=row["value"])
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
